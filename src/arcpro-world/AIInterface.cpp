...
{
	ARCPRO_ASSERT(at == AITYPE_PET || at == AITYPE_TOTEM);
...
void AIInterface::_UpdateTargets()
{
	...
	if(m_updateAssist)
	{
		...
		{
			i2 = i++;
			if((*i2) == NULL || (*i2)->event_GetCurrentInstanceId() != m_Unit->event_GetCurrentInstanceId() || !(*i2)->isAlive() || m_Unit->GetDistanceSq((*i2)) >= 2500.0f || !(*i2)->CombatStatus.IsInCombat() || !((*i2)->m_phase & m_Unit->m_phase))
				m_assistTargets.erase(i2);
		}
	}
	...
	if(m_updateTargets)
	{
		...
		if(m_aiTargets.size() == 0 && m_AIState != STATE_IDLE && m_AIState != STATE_FOLLOWING && m_AIState != STATE_EVADE && m_AIState != STATE_FEAR && m_AIState != STATE_WANDER && m_AIState != STATE_SCRIPTIDLE)
		{
			if(m_Unit->GetMapMgr() && m_Unit->GetMapMgr()->GetMapInfo())
			{
				Unit* target = NULL;
...
void AIInterface::_UpdateCombat(uint32 p_time)
{
	...
	if(m_AIType != AITYPE_PET && m_AIState != STATE_EVADE && m_AIState != STATE_SCRIPTMOVE && !m_is_in_instance
	...
	if(cansee && getNextTarget() && getNextTarget()->isAlive() && m_AIState != STATE_EVADE && !m_Unit->IsCasting())
	{
		...
		switch(agent)
		{
			...
			case AGENT_SPELL:
			{
				...
				if(los && ((distance <= m_nextSpell->maxrange + m_Unit->GetModelHalfSize() /*&& distance >= m_nextSpell->minrange*/) || m_nextSpell->maxrange == 0)) // Target is in Range -> Attack
				{
...
bool AIInterface::UnsafeCanOwnerAttackUnit(Unit* pUnit)
{
	...
	//don't agro neutrals
	if((pUnit->IsPlayer() || pUnit->IsPet()) && m_Unit->m_factionDBC && m_Unit->m_factionDBC->RepListId == -1 && m_Unit->m_faction->HostileMask == 0 && m_Unit->m_faction->FriendlyMask == 0)
		return false;
	else if((m_Unit->IsPlayer() || m_Unit->IsPet()) && pUnit->m_factionDBC && pUnit->m_factionDBC->RepListId == -1 && pUnit->m_faction->HostileMask == 0 && pUnit->m_faction->FriendlyMask == 0)
		return false;
...
Unit* AIInterface::FindTarget()
{
	...
	if(target)
	{
		...
		if(target->GetCreatedByGUID() != 0)
		{
			...
			Unit* target2 = m_Unit->GetMapMgr()->GetPlayer(Arcpro::Util::GUID_LOPART(charmer));
...
void AIInterface::_CalcDestinationAndMove(Unit* target, float dist)
{
	...
	if(target != NULL)
	{
		...
		if(abs(m_last_target_x - newx) < DISTANCE_TO_SMALL_TO_WALK && abs(m_last_target_y - newy) < DISTANCE_TO_SMALL_TO_WALK && m_creatureState == MOVING)
			return;
	...
	else if(m_creatureState == STOPPED && (m_AIState == STATE_IDLE || m_AIState == STATE_SCRIPTMOVE) && !m_moveTimer && !m_timeToMove && getUnitToFollow() == NULL) //creature is stopped and out of Combat
	{
		...
		if(getUnitToFollow() == NULL)
		{
				...
				/*if(m_moveRandom)
				{
					if((rand()%10)== 0)
					{
						...
						if(m_Unit->CalcDistance(m_Unit->GetPositionX(), m_Unit->GetPositionY(), m_Unit->GetPositionZ(), ((Creature*)m_Unit)->respawn_cord[0], ((Creature*)m_Unit)->respawn_cord[1], ((Creature*)m_Unit)->respawn_cord[2]) >15)
							MoveTo(((Creature*)m_Unit)->respawn_cord[0],((Creature*)m_Unit)->respawn_cord[1],((Creature*)m_Unit)->respawn_cord[2],false); //return home
						else
			...
			else //we do have waypoints
			{
				...
				{
					++m_currentWaypoint;
					if(m_currentWaypoint > GetWayPointsCount()) //hmm maybe we should stop being path walker since we are waiting here anyway
						destpoint = -1;
					else
						destpoint = m_currentWaypoint;
				}
...
//			uint8 wt = m_Unit->GetMapMgr()->GetWaterType(Fx, Fy);
			...
			if(sWorld.Collision)
			{
				...
				{
					if(CollideInterface.GetFirstPoint(m_Unit->GetMapId(), m_Unit->GetPositionX(), m_Unit->GetPositionY(), m_Unit->GetPositionZ() + 2.0f, Fx, Fy, Fz + 2.0f, Fx, Fy, Fz, -1.0f))
					{
						//Fz = CollideInterface.GetHeight(m_Unit->GetMapId(), Fx, Fy, m_Unit->GetPositionZ() + 2.0f);
					}
				}

				if(fabs(m_Unit->GetPositionZ() - Fz) > 10.0f || (wl != 0.0f && Fz < wl)) // in water
...
void AIInterface::EventChangeFaction(Unit* ForceAttackersToHateThisInstead)
{
	...
	{
		for(set<Object*>::iterator itr = m_Unit->GetInRangeSetBegin(); itr != m_Unit->GetInRangeSetEnd(); ++itr)
			if((*itr)->IsUnit() && TO<Unit*>(*itr)->GetAIInterface() && TO<Unit*>(*itr)->GetAIInterface()->getThreatByPtr(m_Unit)) //this guy will join me in fight since I'm telling him "sorry i was controlled"
			{
...
{
	ARCPRO_ASSERT(totemspell != 0);
	if(p_time >= m_totemspelltimer)
	{
		Spell* pSpell = sSpellFactoryMgr.NewSpell(m_Unit, totemspell, true, 0);
		Unit* nextTarget = getNextTarget();
		if(nextTarget == NULL || (!m_Unit->GetMapMgr()->GetUnit(nextTarget->GetGUID()) || !nextTarget->isAlive() || !IsInrange(m_Unit, nextTarget, pSpell->GetProto()->base_range_or_radius_sqr) || !isAttackable(m_Unit, nextTarget, !(pSpell->GetProto()->c_is_flags & SPELL_FLAG_IS_TARGETINGSTEALTHED))))
		{
...
void AIInterface::EventUnitDied(Unit* pUnit, uint32 misc1)
{
	...
	if(m_Unit->GetMapMgr() && m_Unit->IsCreature() && !m_Unit->IsPet() && pInstance && (pInstance->m_mapInfo->type == INSTANCE_RAID || pInstance->m_mapInfo->type == INSTANCE_NONRAID || pInstance->m_mapInfo->type == INSTANCE_MULTIMODE))
	{
