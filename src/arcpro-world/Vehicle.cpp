...
void Vehicle::AddPassengerToSeat(Unit* passenger, uint32 seatid)
{
	...
	if(passenger->IsCreature())
	{
		Creature* pCreature = static_cast<Creature*>(passenger);
		
		if(pCreature->GetScript() != NULL)
			pCreature->GetScript()->OnEnterVehicle();
	}

	if(owner->IsCreature())
	{
		Creature* pCreature = static_cast<Creature*>(owner);

		if(pCreature->GetScript() != NULL)
		{
			if(passengercount == 1)
				pCreature->GetScript()->OnFirstPassengerEntered(passenger);

			if(!HasEmptySeat())
				pCreature->GetScript()->OnVehicleFull();
		}
	}
}
...
void Vehicle::EjectPassengerFromSeat(uint32 seatid)
{
	...
	if(passenger->IsCreature())
	{
		...
		if(pCreature->GetScript() != NULL)
			pCreature->GetScript()->OnExitVehicle();
	}
	if(owner->IsCreature())
	{
		Creature* pCreature = static_cast<Creature*>(owner);
		...
		{
			if(passengercount == 0)
				pCreature->GetScript()->OnLastPassengerLeft(passenger);
		}else{
			// The passenger summoned the vehicle, and we have no script to remove it, so we remove it here
			if((passengercount == 0) && (pCreature->GetSummonedByGUID() == passenger->GetGUID()))
				pCreature->Despawn(1 * 1000, 0);
		}
	}
}
...
void Vehicle::EjectAllPassengers()
{
	for(uint32 i = 0; i < MAX_VEHICLE_SEATS; i++)
		...
		{
			Unit* pUnit = owner->GetMapMgr()->GetUnit(seats[i]->GetPassengerGUID());
			if(pUnit == NULL)
			{
			...
			if(pUnit->GetVehicleComponent() != NULL)
...
void Vehicle::InstallAccessories()
{
	...
	for(std::vector<VehicleAccessoryEntry*>::iterator itr = v->begin(); itr != v->end(); ++itr)
	{
		...
		CreatureInfo* pCreautreInfo = CreatureNameStorage.LookupEntry(accessory->accessory_entry);
		if(pCreautreInfo == NULL)
			continue;
		
		CreatureProto* pCreatureProto = CreatureProtoStorage.LookupEntry(accessory->accessory_entry);
		if(pCreatureProto == NULL)
			continue;
		
		Creature* pCreature = owner->GetMapMgr()->CreateCreature(accessory->accessory_entry);
		pCreature->Load(pCreatureProto, owner->GetPositionX(), owner->GetPositionY(), owner->GetPositionZ(), owner->GetOrientation());
		pCreature->transporter_info.guid = owner->GetGUID();
		pCreature->transporter_info.seat = accessory->seat;
		pCreature->Phase(PHASE_SET, owner->GetPhase());
		pCreature->SetFaction(owner->GetFaction());
		pCreature->PushToWorld(owner->GetMapMgr());
		
		AddPassengerToSeat(pCreature, accessory->seat);
		installed_accessories.push_back(pCreature->GetGUID());
	}
}
...
void Vehicle::RemoveAccessories()
{
	...
	{
		Unit* pUnit = owner->GetMapMgr()->GetUnit(*itr);
		if(pUnit == NULL)
			continue;
