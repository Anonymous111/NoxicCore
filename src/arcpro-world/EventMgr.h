...
{
	TimedEvent(void* object, CallbackBase* callback, uint32 type, time_t time, uint32 repeat, uint32 flags) : obj(object), cb(callback), eventType(type), eventFlag(static_cast< uint16 >(flags)), msTime(time), currTime(time), repeats(static_cast< uint16 >(repeat)), deleted(false), ref(0) {}
...
public:
	...
	template <class Class> void RemoveEvents(Class* obj, int32 type) { obj->event_RemoveEvents(type); }
	template <class Class> void ModifyEventTimeLeft(Class* obj, uint32 type, time_t time, bool unconditioned = true) { obj->event_ModifyTimeLeft(type, time, unconditioned); }
	template <class Class> void ModifyEventTimeAndTimeLeft(Class* obj, uint32 type, time_t time) { obj->event_ModifyTimeAndTimeLeft(type, time); }
	template <class Class> void ModifyEventTime(Class* obj, uint32 type, time_t time) { obj->event_ModifyTime(type, time); }
	template <class Class> bool HasEvent(Class* obj, uint32 type) { return obj->event_HasEvent(type); }
