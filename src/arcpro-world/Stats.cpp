...
uint32 CalculateXpToGive(Unit* pVictim, Unit* pAttacker)
{
	...
	{
		// based on: http://www.wowpedia.org/Talk:Formulas:Mob_XP#Hunter.27s_pet_XP (2008/01/12)
		uint32 ownerLvl = TO<Pet*>(pAttacker)->GetPetOwner()->getLevel();
		VictimLvl += ownerLvl - AttackerLvl;
		AttackerLvl = ownerLvl;
	}
	else if((int32)VictimLvl - (int32)AttackerLvl > 10) //not blizzlike but more balanced
		return 0;
