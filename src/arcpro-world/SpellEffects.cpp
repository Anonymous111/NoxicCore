...
void Spell::SpellEffectInstantKill(uint32 i)
{
	...
	*/
	switch(GetProto()->Id)
	{
		...
		{
			unitTarget->GetPowerType() == POWER_TYPE_RAGE ? m_caster->DealDamage(unitTarget, m_spellInfo->EffectBasePoints[0], 0, spell_id, false) : m_caster->DealDamage(unitTarget, m_spellInfo->EffectBasePoints[1], 0, spell_id, false);
			return;
		}break;*/
		/*case 36484: // Mana Burn same like Power Burn
		{
			m_caster->DealDamage(unitTarget, m_spellInfo->EffectBasePoints[0], 0, spell_id, false);
			return;
		}break;*/
		...
		case 18788: //Demonic Sacrifice (508745)
			...
			if(spell_id1 && spell_id1 != 0)
					u_caster->CastSpell(u_caster, dbcSpell.LookupEntryForced(spell_id1), true);
		break;
	}
...
void Spell::SpellEffectSchoolDMG(uint32 i) // dmg school
{
	...
	else
	{
		...
		switch(GetProto()->Id)
		{
			...
			{
				if(p_caster != NULL) // Half of Attack Power
			...
			case 23922:
			case 23923: // Shield Slam - damage is increased by block value
			case 23924:
			case 23925:
			case 25258:
			case 30356:
			...
			case 49052:
			{
				if(p_caster != NULL)
				{
					...
					if(pItem)
						bowdmg = (pItem->GetProto()->Damage[0].Min + pItem->GetProto()->Damage[0].Max) * 0.2f; //+unmodified weapon damage
					else
						bowdmg = 0;
					if(pItemProto)
						ammodmg = (pItemProto->Damage[0].Min + pItemProto->Damage[0].Max) * 0.2f; //+unmodified ammo damage
					else
						ammodmg = 0;
			...
			case 48668:
			{
				if(p_caster != NULL)
				{
					...
					dmg += float2int32(p_caster->GetAP() * (perc * 0.01f));
					if(p_caster->HasDummyAura(SPELL_HASH_CUT_TO_THE_CHASE))
					{
						Aura* pAura = p_caster->FindPositiveAuraByNameHash(SPELL_HASH_SLICE_AND_DICE);
						if(pAura)
						{
							pAura->SetDuration(21000);
							pAura->SetTimeLeft(21000);
						}
					}
				}
			}break;*/
...
void Spell::SpellEffectDummy(uint32 i) // Dummy(Scripted events)
{
	...
	switch(GetProto()->Id)
	{
		case 49576:
		{
			...
			p_caster->CastSpell(unitTarget, 51399, true);
		...
		case 60970:
		{
			...
			uint32 ClearSpellId[5] = { 20252, 20616, 20617, 25272, 25275,};
			for(i = 0; i < 5; i++)
			{
		...
		case 47471:
		{
			...
			{                
				...
				unitTarget->RemoveAura(suddenDeath);
		...
		/*case 35729:
		{
			...
			{
				pAura = unitTarget->m_auras[i];
				if(pAura != NULL && !pAura->IsPassive() && !pAura->IsPositive() && !(pAura->GetSpellProto()->Attributes & ATTRIBUTES_IGNORE_INVULNERABILITY) && pAura->GetSpellProto()->School != 0)
					unitTarget->RemoveAuraBySlot(i);
			}
		}break;*/
		case 5938:
			...
			if(p_caster->GetGroup() != NULL)
			{
				...
				if(grp->m_prayerOfMendingCount > 0)
				{
					...
					for(uint32 sgid = 0; sgid < grp->GetSubGroupCount(); ++sgid)
					{
						...
						{
							if((*itr)->m_loggedInPlayer != NULL && (*itr)->m_loggedInPlayer->GetInstanceID() == p_caster->GetInstanceID() && (*itr)->m_loggedInPlayer != p_caster && (*itr)->m_loggedInPlayer->GetDistance2dSq(p_caster) <= 400.0f)
		...
		case 55198:
		{
			...
			SpellEffectTriggerSpell(i);
			SpellEffectTriggerSpell(i); // Hacks!
		}break;
		case 25228:
		{
		...
		case 49895:
		{
			...
			if(isAttackable(u_caster, unitTarget, false))
				u_caster->SpellNonMeleeDamageLog(unitTarget, spell_id, damage, true);
			else if(unitTarget->IsCreature())
			{
				CreatureInfo* ci = TO_CREATURE(unitTarget)->GetCreatureInfo();
				if(ci && ci->Type == RACE_UNDEAD)
					u_caster->Heal(unitTarget, spell_id, float2int32(damage * 1.5f));
			}
			else if(unitTarget->IsPlayer())
			{        
				if(playerTarget->HasActiveAura(49039))
					u_caster->Heal(playerTarget, spell_id, float2int32(damage * 1.5f));
			}
		}break;*/
		case 37336:
		{
			...
			switch(shape)
			{
				case FORM_NORMAL: NewSpell = 37344; break;
				case FORM_BEAR: NewSpell = 37340; break;
				case FORM_DIREBEAR: NewSpell = 37340; break;
				case FORM_CAT: NewSpell = 37341; break;
				case FORM_TREE: NewSpell = 37342; break;
				case FORM_MOONKIN: NewSpell = 37343; break;
			}
			if(NewSpell != 0)
		...
		case 57946:
		{
			...
			uint32 mod;
			if(m_spellInfo->Id == 1454)
				mod = 2;
			else if(m_spellInfo->Id == 1455)
				mod = 3;
			else if(m_spellInfo->Id == 1456)
				mod = 4;
			else if(m_spellInfo->Id == 11687)
				mod = 5;
			else
				mod = 6;
			...
			if(damage >= playerTarget->GetUInt32Value(UNIT_FIELD_HEALTH))
				return;
			p_caster->DealDamage(playerTarget,damage,0,0,spell_id);
			damage = damage * (100 + playerTarget->m_lifetapbonus) / 100;
		...
		/*case 23725:
		{
			...
			Spell* spe = CREATESPELL(u_caster,inf,true,NULL);
			spe->prepare(&tgt);
		}break;*/
		/*case 12975:
		{
		...
		case 10137:
		{
			...
			//You must whistle this every time you reach there to make Fizzule ally to you.
		}break;
		case 11540:
		{
...
void Spell::SpellEffectTeleportUnits(uint32 i)    // Teleport Units
{
	...
	{
		LOG_ERROR("Spell %u (%s) has a teleport effect, but has no teleport flag.", spell_id, m_spellInfo->Name);
		return;
	}
	...
	{
		TeleportCoords* TC = ::TeleportCoordStorage.LookupEntry(spell_id);

		if(TC == NULL)
		{
			LOG_ERROR("Spell %u (%s) has a TELEPORT TO COORDINATES effect, but has no coordinates to teleport to. ", spell_id, m_spellInfo->Name);
			return;
		}
...
void Spell::SpellEffectApplyAura(uint32 i)  // Apply Aura
{
	...
	else
	{
		if(unitTarget->IsPlayer() && unitTarget->IsInWorld() && TO<Player*>(unitTarget)->GetSession() && TO<Player*>(unitTarget)->GetSession()->CanUseCommand('z'))
			sChatHandler.BlueSystemMessage(TO<Player*>(unitTarget)->GetSession(), "[%sSystem%s] |rSpell::SpellEffectApplyAura: %s EffectApplyAuraName [%u] .", MSG_COLOR_WHITE, MSG_COLOR_LIGHTBLUE, MSG_COLOR_SUBWHITE, i);
	}
#endif
	...
	else
	{
		...
		switch(GetProto()->Id)
		{
			...
			case 34299: //Druid: Improved Leader of the Pack
			{
				...
				Player* mPlayer = TO<Player*>(unitTarget);
				if(!mPlayer->IsInFeralForm() || (mPlayer->GetShapeShift() != FORM_CAT &&
						mPlayer->GetShapeShift() != FORM_BEAR && mPlayer->GetShapeShift() != FORM_DIREBEAR))
					break;
			...
			case 22845: // Druid: Frenzied Regeneration
			{
				if(!unitTarget->IsPlayer() || !unitTarget->isAlive())
					break;
				Player* mPlayer = TO<Player*>(unitTarget);
				if(!mPlayer->IsInFeralForm() || (mPlayer->GetShapeShift() != FORM_BEAR && mPlayer->GetShapeShift() != FORM_DIREBEAR))
					break;
...
void Spell::SpellEffectWeapon(uint32 i)
{
	...
	if(skill)
	{
		...
		{
			// unhandled.... if we have the skill line
		}
	}
}
...
void Spell::SpellEffectTriggerMissile(uint32 i) // Trigger Missile
{
	...
	uint32 spell_id = GetProto()->EffectTriggerSpell[i];
	if(spell_id == 0)
	{
	...
	SpellEntry* spInfo = dbcSpell.LookupEntryForced(spell_id);
	if(spInfo == NULL)
	{
...
void Spell::SpellEffectOpenLock(uint32 i) // Open Lock
{
	...
	switch(locktype)
	{
		...
		case LOCKTYPE_SLOW_OPEN: // used for BG go's
		{
			...
			uint32 spell_id = !gameObjTarget->GetInfo()->Unknown1 ? 23932 : gameObjTarget->GetInfo()->Unknown1;
			SpellEntry* en = dbcSpell.LookupEntry(spell_id);
...
void Spell::SpellEffectLearnSpell(uint32 i) // Learn Spell
{
	...
	if(GetProto()->Id == 483 || GetProto()->Id == 55884)          // "Learning"
	{
		...
		for(int j = 0; j < 5; ++j)
		{
			...
			{
				spell_id = i_caster->GetProto()->Spells[j].Id;
				break;
			}
		}

		if(!spell_id || !dbcSpell.LookupEntryForced(spell_id)) return;

		// learn me!
		p_caster->addSpell(spell_id);
	...
	if(playerTarget)
	{
		...
		for(uint32 j = 0; j < 3; j++)
			if(spellinfo->Effect[j] == SPELL_EFFECT_WEAPON || spellinfo->Effect[j] == SPELL_EFFECT_PROFICIENCY || spellinfo->Effect[j] == SPELL_EFFECT_DUAL_WIELD)
			{
...
void Spell::SpellEffectAddHonor(uint32 i)
{
	...
	if(GetProto()->AttributesExB & ATTRIBUTESEXB_UNK4)
		val /= 10;
...
void Spell::SpellEffectEnchantItem(uint32 i) // Enchant Item Permanent
{
	...
	// Vellums
	if(GetProto()->EffectItemType[i] && (itemTarget->GetEntry() == 39349 || itemTarget->GetEntry() == 39350 || itemTarget->GetEntry() == 43146 ||
			itemTarget->GetEntry() == 38682 || itemTarget->GetEntry() == 37602 || itemTarget->GetEntry() == 43145))
	{
		uint32 itemid = GetProto()->EffectItemType[i];
...
void Spell::SpellEffectInterruptCast(uint32 i) // Interrupt Cast
{
	...
	if(u_caster->HasAura(13867)) //Improved Kick - Silence bug fix
	...
	if(TargetSpell)
	{
		...
		if(school && (TargetSpell->getState() == SPELL_STATE_CASTING
			...
			|| (TargetSpell->GetProto()->ChannelInterruptFlags & CHANNEL_INTERRUPT_ON_4)))
		{
			...
			else // Prevent unit from casting in that school
...
{
	//spell_id 1725 Distract:Throws a distraction attracting the all monsters for ten sec's
	if(!unitTarget || !unitTarget->isAlive())
		return;
...
void Spell::SpellEffectDispelMechanic(uint32 i)
{
	...
	*/
	...
	//	playerTarget->RemoveAura(playerTarget->m_Mountspell_id);
}
...
void Spell::SpellEffectForgetSpecialization(uint32 i)
{
	...
	uint32 spell_id = GetProto()->EffectTriggerSpell[i];
	playerTarget->removeSpell(spell_id, false, false, 0);

	LOG_DEBUG("Player %u have forgot spell %u from spell %u (caster: %u)", playerTarget->GetLowGUID(), spell_id, GetProto()->Id, m_caster->GetLowGUID());
}
...
{
	if(!unitTarget || !unitTarget->IsPet() || !TO<Pet*>(unitTarget)->GetPetOwner() || TO<Pet*>(unitTarget)->GetPetOwner()->getClass() != HUNTER)
		return;
