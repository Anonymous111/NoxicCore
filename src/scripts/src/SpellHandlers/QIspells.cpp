...
bool SummonEkkorash(uint32 i, Spell* pSpell)
{
	...
	sEAS.SpawnCreature(pPlayer, 19493, pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), 0, 0);
	return true;
}
...
bool BalanceMustBePreserved(uint32 i, Aura* pAura, bool apply)
{
	...
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(9720);
	if(pQuest == NULL)
		return true;
...
bool BlessingofIncineratus(uint32 i, Spell* pSpell)
{
	...
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(9805);
	if(pQuest == NULL)
		return true;
...
bool TagMurloc(uint32 i, Aura* pAura, bool apply)
{
	...
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(9629);
	if(pQuest == NULL)
		return true;
...
bool CookingPot(uint32 i, Spell* pSpell)
{
	...
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(11379);
	if(pQuest == NULL)
		return true;
...
bool UnyieldingBattleHorn(uint32 i, Spell* pSpell)
{
	...
	Creature* pCreature = pSpell->p_caster->GetMapMgr()->GetInterface()->SpawnCreature(19862, -1190.856079f, 2261.246582f, 46.625797f, 1.705882f, true, false, 0, 0);
	pCreature->Despawn(300000, 0); // 5 mins delay
...
bool YennikuRelease(uint32 i, Spell* pSpell)
{
	...
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(592);
	if(pQuest == NULL)
		return true;
...
bool ScrollOfMyzrael(uint32 i, Spell* pSpell)
{
	...
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(656);
	if(pQuest == NULL)
		return true;
...
bool TheBaitforLarkorwi1(uint32 i, Spell* pSpell)
{
	...
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(4292);
	if(pQuest == NULL)
		return true;
...
bool Fumping(uint32 i, Spell* pSpell)
{
	...
	Creature* pCreature = sEAS.SpawnCreature(pPlayer, entry, pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), 5 * 60 * 1000);

	if(entry == 22483) //Sand Gnomes ;)
		pCreature->SendChatMessage(CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, "YIEEEEEEEAA!");
...
bool WelcomingtheWolfSpirit(uint32 i, Spell* pSpell)
{
	...
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(10791);
	if(pQuest == NULL)
		return true;
...
bool FloraoftheEcoDomes(uint32 i, Spell* pSpell)
{
	...
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(10426);
	if(pQuest == NULL)
		return true;
...
bool AdministreringtheSalve(uint32 i, Aura* pAura, bool apply)
{
	...
	if(apply)
	{
		...
		QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(9447);
...
bool ZappedGiants(uint32 i, Spell* pSpell)
{
	...
	Creature* pCreature = TO_CREATURE(pSpell->GetUnitTarget());
	if(pCreature == NULL)
		return true;
		...
		case 5360:
			{
				...
				Creature* zappedpCreature = sEAS.SpawnCreature(pPlayer, 14639, X, Y, Z, 0);
				zappedpCreature->Despawn(3 * 60 * 1000, 0);
			}
			break;
		case 5361:
			{
				pCreature->Despawn(1000, 6 * 60 * 1000);
				Creature* zappedpCreature = sEAS.SpawnCreature(pPlayer, 14638, X, Y, Z, 0);
				zappedpCreature->Despawn(3 * 60 * 1000, 0);
			}
			break;
		case 5359:
			{
				pCreature->Despawn(1000, 6 * 60 * 1000);
				Creature* zappedpCreature = sEAS.SpawnCreature(pPlayer, 14603, X, Y, Z, 0);
				zappedpCreature->Despawn(3 * 60 * 1000, 0);
			}
			break;
		case 5358:
			{
				pCreature->Despawn(1000, 6 * 60 * 1000);
				Creature* zappedpCreature = sEAS.SpawnCreature(pPlayer, 14640, X, Y, Z, 0);
				zappedpCreature->Despawn(3 * 60 * 1000, 0);
			}
			break;
		case 5357:
			{
				pCreature->Despawn(1000, 6 * 60 * 1000);
				Creature* zappedpCreature = sEAS.SpawnCreature(pPlayer, 14604, X, Y, Z, 0);
				zappedpCreature->Despawn(3 * 60 * 1000, 0);
			}
			break;
	}
...
bool RodofPurification(uint32 i, Spell* pSpell)
{
	...
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(10839);
	if(pQuest == NULL)
		return true;
...
bool WyrmcallersHorn(uint32 i, Spell* pSpell)
{
	...
	Creature* pCreature = sEAS.SpawnCreature(pPlayer, 24019, pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), 0, 0);
	if(pCreature == NULL)
		return true;
...
bool RaeloraszSpark(uint32 i, Spell* pSpell)
{
	...
	Creature* pCreature = sEAS.SpawnCreature(pPlayer, 26237, pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), 0, 0);
	pCreature->Despawn(5 * 60 * 1000, 0);

	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(11969);
	if(pQuest == NULL)
		return true;
...
bool RuneOfDistortion(uint32 i, Spell* pSpell)
{
	...
	Creature* pCreature = sEAS.SpawnCreature(pPlayer, 32162, pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), 0, 0);
	pCreature->Despawn(5 * 60 * 1000, 0);
...
bool GoreBladder(uint32 i, Spell* pSpell)
{
	...
	Player* pPlayer = pSpell->p_caster;
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(12810);
...
bool PlagueSpray(uint32 i, Spell* pSpell)
{
	...
	Player* pPlayer = pSpell->p_caster;
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(11307);
	if(pQuest && pQuest->GetMobCount(0) < pQuest->GetQuest()->required_mobcount[0])
	{
		pQuest->SetMobCount(0, pQuest->GetMobCount(0) + 1);
		pQuest->SendUpdateAddKill(0);
...
bool PurifiedAshes(uint32 i, Spell* pSpell)
{
	...
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(entry);
	if(pQuest && pQuest->GetMobCount(0) < pQuest->GetQuest()->required_mobcount[0])
	{
		pQuest->SetMobCount(0, pQuest->GetMobCount(0) + 1);
		pQuest->SendUpdateAddKill(0);
...
bool CraftyBlaster(uint32 i, Spell* pSpell)
{
	...
	Player* pPlayer = pSpell->p_caster;
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(11653);
	if(pQuest && pQuest->GetMobCount(0) < pQuest->GetQuest()->required_mobcount[0])
	{
		pQuest->SetMobCount(0, pQuest->GetMobCount(0) + 1);
		pQuest->SendUpdateAddKill(0);
...
bool RagefistTorch(uint32 i, Spell* pSpell)
{
	...
	Player* pPlayer = pSpell->p_caster;
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(11593);
	if(pQuest && pQuest->GetMobCount(0) < pQuest->GetQuest()->required_mobcount[0])
	{
		pQuest->SetMobCount(0, pQuest->GetMobCount(0) + 1);
		pQuest->SendUpdateAddKill(0);
...
bool HodirsHorn(uint32 i, Spell* pSpell)
{
	...
	Player* pPlayer = pSpell->p_caster;
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(12977);
	if(pQuest && pQuest->GetMobCount(0) < pQuest->GetQuest()->required_mobcount[0])
	{
		pQuest->SetMobCount(0, pQuest->GetMobCount(0) + 1);
		pQuest->SendUpdateAddKill(0);
...
bool TelluricPoultice(uint32 i, Spell* pSpell)
{
	...
	Player* pPlayer = pSpell->p_caster;
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(12937);
	if(pQuest && pQuest->GetMobCount(0) < pQuest->GetQuest()->required_mobcount[0])
	{
		pQuest->SetMobCount(0, pQuest->GetMobCount(0) + 1);
		pQuest->SendUpdateAddKill(0);
...
bool Screwdriver(uint32 i, Spell* pSpell)
{
	...
	Player* pPlayer = pSpell->p_caster;
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(11730);
	if(pQuest && pQuest->GetMobCount(0) < pQuest->GetQuest()->required_mobcount[0])
	{
		pQuest->SetMobCount(0, pQuest->GetMobCount(0) + 1);
		pQuest->SendUpdateAddKill(0);
...
bool IncineratingOil(uint32 i, Spell* pSpell)
{
	...
	Player* pPlayer = pSpell->p_caster;
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(12568);
	if(pQuest && pQuest->GetMobCount(0) < pQuest->GetQuest()->required_mobcount[0])
	{
		pQuest->SetMobCount(0, pQuest->GetMobCount(0) + 1);
		pQuest->SendUpdateAddKill(0);
...
bool PrayerBeads(uint32 i, Spell* pSpell)
{
	...
	Player* pPlayer = pSpell->p_caster;
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(10935);
	if(pQuest && pQuest->GetMobCount(0) < pQuest->GetQuest()->required_mobcount[0])
	{
		pQuest->SetMobCount(0, pQuest->GetMobCount(0) + 1);
		pQuest->SendUpdateAddKill(0);
...
{
	QuestLogEntry* pQuest = s->p_caster->GetQuestLogForEntry(9427);
		...
		else if(!a->GetTimeLeft())
		{
			...
			if(Rand(75.0f))    // 75% chance on success
			{
				...
				if(m_target->IsCreature())
				{
					...
					QuestLogEntry* pQuest = p_caster->GetQuestLogForEntry(tamequest->id);
					if(pQuest != NULL)
					{
						pQuest->SetMobCount(0, 1);
						pQuest->SendUpdateAddKill(1);
...
bool ArcaneDisruption(uint32 i, Aura* pAura, bool apply)
{
	...
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(13149);
	if(!pQuest)
		return false;

	GameObject* crate = pPlayer->GetMapMgr()->GetInterface()->GetGameObjectNearestCoords(pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), 190094);
	if(crate)
	{
		GameObject* go = pPlayer->GetMapMgr()->CreateGameObject(190095);
		...
		{
			//weee, Uther
			CreatureProto* pCreatureProto = CreatureProtoStorage.LookupEntry(26528);
			CreatureInfo* pCreatureInfo = CreatureNameStorage.LookupEntry(26528);
			Creature* pCreature = NULL;
			if(pCreatureProto && pCreatureInfo)
			{
				pCreature = pPlayer->GetMapMgr()->CreateCreature(26528);
				if(pCreature)
				{
					//position is guessed
					pCreature->Load(pCreatureProto, 1759.4351f, 1265.3317f, 138.052f, 0.1902f);
					pCreature->PushToWorld(pPlayer->GetMapMgr());
				}
			}
		}
	}
...
bool CurativeAnimalSalve(uint32 i, Spell* pSpell) // Curing the Sick
{
	...
	if(target->GetEntry() == 12296 || target->GetEntry() == 12298)
	{
		...
		if(pQuest == NULL)
		{
			pQuest = caster->GetQuestLogForEntry(6124);
			if(pQuest == NULL)
				return true;
		}
...
bool SymbolOfLife(uint32 i, Spell* pSpell) // Alliance ress. quests
{
	...
	QuestLogEntry* pQuest;

	for(uint8 j = 0; j < 3; j++)
	{
		if(pPlayer->GetQuestLogForEntry(quests[j]) != NULL)
		{
			pQuest = pPlayer->GetQuestLogForEntry(quests[j]);
			questOk = true;
...
bool FilledShimmeringVessel(uint32 i, Spell* pSpell) // Blood Elf ress. quest
{
	...
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(9685);
	if(pQuest == NULL)
		return true;
...
bool DouseEternalFlame(uint32 i, Spell* pSpell)
{
	...
	Player* pPlayer = pSpell->p_caster;
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(9737);
	if(pQuest == NULL)
		return true;

	GameObject* Flame = pPlayer->GetMapMgr()->GetInterface()->GetGameObjectNearestCoords(3678, -3640, 139, 182068);
	if(Flame != NULL)
	{
...
bool Triage(uint32 i, Spell* pSpell)
{
	...
	if(!pSpell->p_caster || pSpell->GetUnitTarget() == NULL)
		return true;

	pSpell->p_caster->CastSpell(pSpell->GetUnitTarget(), dbcSpell.LookupEntry(746), true);
	QuestLogEntry* pQuest = pSpell->p_caster->GetQuestLogForEntry(6624);
...
bool PoweringOurDefenses(uint32 i, Spell* pSpell)
{
	...
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(8490);
	if(pQuest == NULL)
		return true;
...
bool WarIsHell(uint32 i, Spell* pSpell)
{
	...
	Player* pPlayer = pSpell->p_caster;
	Creature* target = pPlayer->GetMapMgr()->GetInterface()->GetCreatureNearestCoords(pPlayer->GetPositionX(), pPlayer->GetPositionY() , pPlayer->GetPositionZ(), 24008);
	if(target == NULL)
		return true;

	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(11270);
	if(pQuest == NULL)
		return true;
	...
	obj = sEAS.SpawnGameobject(pPlayer, 183816, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), target->GetOrientation(), 1, 0, 0, 0, 0);
...
bool ConvertingSentry(uint32 i, Spell* pSpell)
{
	...
	pQuest = pCaster->GetQuestLogForEntry(11525);
	if(pQuest == NULL)
	{
		pQuest = pCaster->GetQuestLogForEntry(11524);
		if(pQuest == NULL)
			return true;
	}
	...
	for(ObjectSet::iterator itr = pSpell->m_caster->GetInRangeSetBegin(); itr != pSpell->m_caster->GetInRangeSetEnd(); ++itr)
	{
		...
		if(pTarget->GetEntry() == 25084)
		{
			if(pQuest->GetMobCount(0) < pQuest->GetQuest()->required_mobcount[0])
			{
				pQuest->SetMobCount(0, pQuest->GetMobCount(0) + 1);
				pQuest->SendUpdateAddKill(0);
				Creature* FreedGreengill = sEAS.SpawnCreature(pPlayer, 25085, pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ(), pTarget->GetOrientation(), 0);
...
bool ShipBombing(uint32 i, Spell* pSpell)
{
	...
	QuestLogEntry* pQuest = NULL;
	pQuest = pPlayer->GetQuestLogForEntry(11542);
	if(pQuest == NULL)
	{
		pQuest = pPlayer->GetQuestLogForEntry(11543);
		if(pQuest == NULL)
		{
			return true;
		}
...
bool LeyLine(uint32 i, Spell* pSpell)
{
	...
	Player* pPlayer = pSpell->p_caster;
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(11547);
	if(pQuest == NULL)
		return true;
	...
	for(uint32 i = 0; i < sizeof(portals) / sizeof(uint32); i++)
	{
		...
		if(portal != NULL && pQuest->GetMobCount(i) < pQuest->GetQuest()->required_mobcount[i])
		{
...
bool ManaRemnants(uint32 i, Spell* pSpell)
{
	...
	{
		pQuest = pPlayer->GetQuestLogForEntry(quests[i]);
		if(pQuest != NULL && pQuest->GetMobCount(0) < pQuest->GetQuest()->required_mobcount[0])
		{
...
bool InducingVision(uint32 i, Spell* pSpell)
{
	...
	Creature* target = mTarget->GetMapMgr()->GetInterface()->SpawnCreature(2983, -2238.994873f, -408.009552f, -9.424423f, 5.753043f, true, false, 0, 0);
	target->GetAIInterface()->setMoveType(11);

	sEAS.CreateCustomWaypointMap(target);

	sEAS.WaypointCreate(target, -2240.521729f, -407.114532f, -9.424648f, 5.753043f, 0, 256, 0);
	sEAS.WaypointCreate(target, -2225.764404f, -419.251404f, -9.360950f, 5.575544f, 0, 256, 0);
	sEAS.WaypointCreate(target, -2200.883789f, -440.999634f, -5.606637f, 5.599899f, 0, 256, 0);
	sEAS.WaypointCreate(target, -2143.712646f, -468.068665f, -9.401191f, 0.797177f, 0, 256, 0);
	sEAS.WaypointCreate(target, -2100.811279f, -420.980194f, -5.322747f, 0.918914f, 0, 256, 0);
	sEAS.WaypointCreate(target, -2079.469482f, -392.465576f, -10.262321f, 0.930689f, 0, 256, 0);
	sEAS.WaypointCreate(target, -2043.699707f, -343.802155f, -6.971242f, 0.934694f, 0, 256, 0);
	sEAS.WaypointCreate(target, -2001.858521f, -242.533966f, -10.763200f, 0.952265f, 0, 256, 0);
	sEAS.WaypointCreate(target, -1924.751343f, -119.969292f, -11.770116f, 0.754775f, 0, 256, 0);
	sEAS.WaypointCreate(target, -1794.804565f, -7.919222f, -9.326016f, 0.711578f, 0, 256, 0);
	sEAS.WaypointCreate(target, -1755.206421f, 72.430847f, 1.121445f, 1.103484f, 0, 256, 0);
	sEAS.WaypointCreate(target, -1734.550049f, 116.842003f, -4.337420f, 3.350100f, 0, 256, 0);
	sEAS.WaypointCreate(target, -1720.041870f, 125.933235f, -2.325089f, 0.903206f, 0, 256, 0);
	sEAS.WaypointCreate(target, -1704.406860f, 183.596405f, 12.065074f, 0.691148f, 0, 256, 0);
	sEAS.WaypointCreate(target, -1674.317261f, 201.597565f, 11.235887f, 0.475164f, 0, 256, 0);
	sEAS.WaypointCreate(target, -1624.068726f, 223.555389f, 2.074259f, 0.416259f, 0, 256, 0);
	sEAS.WaypointCreate(target, -1572.863403f, 234.714813f, 2.305119f, 0.781471f, 0, 256, 0);
	sEAS.WaypointCreate(target, -1542.866943f, 277.896759f, 20.543165f, 1.256637f, 0, 256, 0);
	sEAS.WaypointCreate(target, -1541.813232f, 316.415649f, 49.910889f, 1.248783f, 0, 256, 0);
	sEAS.WaypointCreate(target, -1526.979126f, 329.664001f, 61.835552f, 1.182024f, 0, 256, 0);
	sEAS.WaypointCreate(target, -1524.173584f, 335.237610f, 63.325703f, 1.173092f, 0, 256, 0);
	sEAS.WaypointCreate(target, -1513.968506f, 355.759338f, 63.064487f, 1.119193f, 0, 256, 0);
	sEAS.EnableWaypoints(target);
	return true;
}
...
bool StoppingTheSpread(uint32 i, Spell* pSpell)
{
	...
	Creature* target = pPlayer->GetMapMgr()->GetInterface()->GetCreatureNearestCoords(pPlayer->GetPositionX(), pPlayer->GetPositionY() , pPlayer->GetPositionZ(), 18240);
	if(target == NULL)
		return true;

	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(9874);
	...
	if(pQuest && pQuest->GetMobCount(0) < pQuest->GetQuest()->required_mobcount[0])
	{
		...
		GameObject* obj = sEAS.SpawnGameobject(pPlayer, 183816, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), target->GetOrientation(), 1, 0, 0, 0, 0);
		sEAS.GameobjectDelete(obj, 1 * 30 * 1000);
	};
...
bool RuthlessCunning(uint32 i, Spell* pSpell)
{
	...
	Creature* kilsorrow = pPlayer->GetMapMgr()->GetInterface()->GetCreatureNearestCoords(pPlayer->GetPositionX(), pPlayer->GetPositionY() , pPlayer->GetPositionZ());
	if(kilsorrow == NULL || kilsorrow->isAlive())
		return true;

	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(9927);
	if(pQuest && pQuest->GetMobCount(0) < pQuest->GetQuest()->required_mobcount[0])
	{
...
bool FindingTheKeymaster(uint32 i, Spell* pSpell)
{
	...
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(10256);
	if(!pQuest)
		return true;
...
bool TheFleshLies(uint32 i, Spell* pSpell)
{
	...
	Player* pPlayer = pSpell->p_caster;
	Creature* target = pPlayer->GetMapMgr()->GetInterface()->GetCreatureNearestCoords(pPlayer->GetPositionX(), pPlayer->GetPositionY() , pPlayer->GetPositionZ(), 20561);

	if(target == NULL)
		return true;

	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(10345);
...
bool SurveyingtheRuins(uint32 i, Spell* pSpell)
{
	...
	if(pQuest->GetMobCount(0) < pQuest->GetQuest()->required_mobcount[0])
	{
		...
		if(mark1 != NULL && pPlayer->CalcDistance(pPlayer, mark1) < 15)
		{
			...
			//if you know a GO which can/should be spawned after marking the point... uncommpQuestt and add GO ID pls :)
...
bool CrystalOfDeepShadows(uint32 i, Spell* pSpell) // Becoming a Shadoweave Tailor
{
	...
	Player* pPlayer = pSpell->p_caster;
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(10833);
...
bool ShatariTorch(uint32 i, Spell* pSpell)
{
	...
	if(pPlayer->CalcDistance(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ()) > 5)
		return true;

	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(10913);
	if(pQuest == NULL)
		return true;
...
bool SpragglesCanteen(uint32 i, Spell* pSpell)
{
	...
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(4492);
	if(pQuest == NULL)
		return true;
...
bool FindingTheSource(uint32 i, Spell* pSpell)
{
	...
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(974);
	if(pQuest == NULL)
		return true;
...
bool ReleaseUmisYeti(uint32 i, Spell* pSpell)
{
	...
	QuestLogEntry* pQuest = pSpell->p_caster->GetQuestLogForEntry(5163);
	if(pQuest == NULL)
		return true;
	...
	{
		if(target->GetEntry() == friends[j] && pQuest->GetMobCount(j) < pQuest->GetQuest()->required_mobcount[j])
		{
			pQuest->SetMobCount(j, 1);
			pQuest->SendUpdateAddKill(j);
			pQuest->UpdatePlayerFields();
			return true;
		}
	}
...
bool ProtectingOurOwn(uint32 i, Spell* pSpell)
{
	...
	Player* pPlayer = pSpell->p_caster;
	QuestLogEntry* pQuest = pPlayer->GetQuestLogForEntry(10488);
	if(pQuest == NULL)
		return true;
...
void SetupQuestItems(ScriptMgr* mgr)
{
	...
	uint32 huntertamingquestspellids[] = { 19548, 19674, 19687, 19688, 19689, 19692, 19693, 19694, 19696, 19697, 19699, 19700, 30099, 30102, 30105, 30646, 30653, 30654, 0 };
	...
	mgr->register_dummy_spell(45834, &HighmessasCleansingSeeds); // Stop the Plague
	mgr->register_dummy_spell(45835, &BixiesInhibitingPowder); // There's Something Going On In Those Caves
	mgr->register_dummy_spell(45536, &CompleteAncestorRitual); // Leading the Ancestors Home
