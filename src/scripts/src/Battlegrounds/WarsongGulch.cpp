...
void WarsongGulch::HookFlagDrop(Player* plr, GameObject* obj)
{
	...
	{
		/* are we returning it? */
		if((obj->GetEntry() == 179785 && plr->IsTeamAlliance()) || (obj->GetEntry() == 179786 && plr->IsTeamHorde()))
		{
...
void WarsongGulch::OnCreate()
{
	...
	SetWorldState(WORLDSTATE_WSG_TIME_ENABLED, 1);
	SetWorldState(WORLDSTATE_WSG_TIME_REMAINING, WSG_MAX_MATCH_TIME - m_timeElapsed);
...
void WarsongGulch::TimerTick()
{
	...
	if(WSG_MAX_MATCH_TIME == m_timeElapsed)
	{
		/* victory! */
		m_ended = true;
		...
		if(m_winningteam == TEAM_ALLIANCE)
			AddHonorToTeam(TEAM_HORDE, 185);
		else
			AddHonorToTeam(TEAM_ALLIANCE, 185);
...
void WarsongGulch::OnStart()
{
	...
	sEventMgr.AddEvent(this, &WarsongGulch::TimerTick, EVENT_UNK, 60000, WSG_MAX_MATCH_TIME, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
